<form id="{{ $formId }}" method="POST" action="{{ $editing ? route('database.update') : route('database.store') }}" class="formularioRegistro">
    @csrf
    ...
    <div class="camposRegistro">
        @foreach ($columns as $column)
            ...
            @endphp
            <label class="campo">{{ $column['name'] }} <small class="textoSuave">{{ $column['type'] }}</small>
    ...
    <div style="margin-top: 1rem"><button class="boton botonPrincipal" type="submit">{{ $editing ? 'Guardar cambios' : 'Crear registro' }}</button></div>
</form>
